...
import { allRecord, deleteRecord, queryRecords } from "@/api/data";
...
export default {
  ...
  data() {
    return {
      ...
      tableLabel: [
        ...
        {
          prop: "AccountTransfer",
          label: "户口转往地址",
        },

        {
          prop: "AddressTransfer",
          label: "档案转往地址",
        },
        {
          prop: "Postcode",
  ...
  methods: {
    ...
    // 列表删除
    delUser(row) {
      this.$confirm("此操作将永久删除该户口档案信息，是否继续？", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
        // 发起请求,删除操作
        deleteRecord(row.sno).then((res) => {
          if (res.data.code === "002") {
            this.$message.error("户口档案信息删除失败");
            return;
          }
          this.$message({
            message: "户口档案信息删除成功",
    ...
    // 获取列表数据
    getList() {
      allRecord(this.config).then((res) => {
        console.log(res);
    ...
    filtrationList() {
      ...
      queryRecords(this.search).then((res) => {
        if (res.data.code == "002") {
          this.tableData = [];
          return;
        }
    ...
    exportExcelSelect() {
      htmlToExcel.getExcel("#selectTable", "户口档案信息表");
    },
  },
  mounted() {
    // 判断登录状态
