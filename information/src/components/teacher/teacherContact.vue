...
import { allContact, deleteContact, queryContacts } from "@/api/data";
...
export default {
  ...
  methods: {
    ...
    // 列表删除
    delUser(row) {
      this.$confirm("此操作将永久删除该学生联系方式，是否继续？", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
        // 发起请求,删除操作
        deleteContact(row.sno).then((res) => {
          if (res.data.code === "002") {
            this.$message.error("学生联系方式删除失败");
            return;
          }
          this.$message({
            message: "学生联系方式删除成功",
    ...
    // 获取列表数据
    getList() {
      allContact(this.config).then((res) => {
    ...
    filtrationList() {
      ...
      queryContacts(this.search).then((res) => {
        if (res.data.code == "002") {
          this.tableData = [];
          return;
        }
    ...
    exportExcelSelect() {
      htmlToExcel.getExcel("#selectTable", "学生联系方式表");
    },
  },
  mounted() {
    // 判断登录状态
